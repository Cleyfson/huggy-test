<template>
  <div class="mb-12">
    <h2 class="text-sm font-medium text-gray-700 mb-4">Segmentação por estado</h2>
      ...
      <div class="w-full md:w-1/2 mt-4 md:mt-0">
        <div class="flex flex-col space-y-2">
          <div v-for="(item, index) in data" :key="index" class="flex items-center">
            ...
            <span class="text-sm text-gray-600">{{ item.state }}</span>
          </div>
        </div>
      </div>
    </div>
...
<script setup>
  ...
  const data = computed(() => insightStore.insights_state);
  ...
  const chartOptions = ref({
    ...
    colors,
    labels: computed(() => data.value.map(item => item.state)),
    legend: { show: false },
    dataLabels: {
      enabled: true,
  ...
  onMounted(async () => {
    await insightStore.getClientsByState();
